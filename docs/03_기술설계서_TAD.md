...
| Command | 입력 | 출력 | 이벤트 |
|---|---|---|---|
| `bootstrap_status` | – | `{ step, progress, ready }` | `bootstrap://progress` |
...
| `preset_ab_test` | `{ presetA, presetB, keyword, seed }` | jobId | `gen://*` |
| `essence_create` | `{ imagePaths[] }` | `{ essencePrompt, tags[] }` | `essence://progress` |
| `style_save / style_delete / styles_list` | Style / id / – | ok / ok / Style[] | – |
| `training_start` | `{ styleId, datasetDir, profile: fast|standard|quality }` | jobId | `train://progress`, `train://sample`, `train://done` |
...
1. 데이터셋 폴더 구성: `training/{style}/dataset/img/{repeat}_{trigger}/*.png` (kohya 규약)
2. `caption.py`로 .txt 캡션 생성 → UI에서 편집 → 저장
3. kohya `sdxl_train_network.py` 실행, 프로파일별 인자 프리셋(fast/standard/quality)은 `training/profiles.toml`에 정의
4. stdout의 step/loss 파싱 → `train://progress` 이벤트, epoch마다 샘플 생성 → `train://sample`
5. 완료 시 .safetensors를 `models/loras/`로 이동, styles.json 등록
