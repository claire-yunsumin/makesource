...
presets.json         styles.json    settings.json (앱 설정 — 안전 네거티브 등)    app.db  logs/
```
...
| `models_list` | – | `ModelEntry[] { name, category, sizeBytes }` (models/ 스캔 — 체크포인트·LoRA 등은 파일 단위, argos/rembg/hf는 폴더 합산) | – |
| `settings_get / settings_save` | – / `{ settings }` | `AppSettings { safeNegative, language: ko\|en }` / ok (settings.json §3) | – |
| `cache_stats / cache_clear` | – / – | `{ sizeBytes }` / `{ freedBytes }` (models/hf — 재다운로드 가능 캐시) | – |
| `licenses_get` | – | `LicenseEntry[] { name, license, url, role }` (resources/licenses.json BOM) | – |
| `model_download / model_set_dir` | … | (F-5.2 저장 위치 변경·개별 다운로드 — 미구현, 후속) | `download://progress` |

프론트는 `src/lib/tauri.ts`에서 위 계약을 타입으로 고정하고, 스토어는 이벤트를 구독해 상태 갱신.

- `engine_health.model_loaded`는 부팅 워밍업(T9.4, docs/11 §P3.3 — 64px 1스텝 더미 잡으로 기본 체크포인트 프리로드) 완료 여부다. 워밍업 전이나 실패 시 false지만 생성은 정상 동작한다(첫 생성이 느릴 뿐).
- `gen://done` 페이로드에는 `durationMs`(생성 전체 소요 ms)가 포함된다 — 성능 계측(T9.1, docs/11 §P0.1). 단계별 소요는 로컬 `logs/perf.log`에 JSON 라인으로 기록되며 프롬프트 등 내용은 남기지 않는다.
